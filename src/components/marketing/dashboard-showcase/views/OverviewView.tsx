...
import type { RangeData, RegionKey, Service, AnomalyRow, RangeKey, Tone } from "../types";
...
function toneToAmountClass(tone: Tone) {
  switch (tone) {
    case "success":
...
export function OverviewView({
  ...
  // We'll use today's tone to tint the "hot service" $ amount
  const hotServiceTone = todayTone;

  return (
    <div className="grid h-full min-h-0 grid-rows-[auto_1fr_1fr] gap-4">
      {/* Metrics */}
        ...
        <div className="min-h-0 rounded-2xl border border-white/10 bg-surface/30 p-4">
          ...
          <div className="mt-4 space-y-1.5">
            {servicesFiltered.map((s) => {
              ...
              const isHot = s.name === activeService;
              const amtClass = isHot
                ? cn(toneToAmountClass(hotServiceTone), "drop-shadow-[0_0_14px_rgba(245,158,11,0.10)]")
                : isActive
                ? "text-foreground/90"
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={cn("font-semibold transition-colors duration-200", amtClass)}>
                      {fmtUsd0(s.amt)}
                    </span>
                  </div>
                  ...
                  <div className="mt-1 h-2 w-full rounded-full bg-white/6 overflow-hidden">
                    <div
                      className={cn(
                        "h-2 rounded-full",
                        isHot
                          ? "bg-linear-to-r from-amber-400/70 via-violet-400/50 to-emerald-400/35"
                          : isActive
                          ? "bg-primary/75"
                          : "bg-white/18"
                      )}
      ...
      <div className="min-h-0 rounded-2xl border border-white/10 bg-surface/30 p-4">
        ...
        <div className="mt-4 min-h-0 overflow-hidden rounded-2xl border border-white/10">
