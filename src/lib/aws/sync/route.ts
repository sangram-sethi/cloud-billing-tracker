...
export async function POST(req: Request) {
  ...
  if (!pulled.ok) {
    await connCol.updateOne(
      ...
      {
        $set: {
          // if creds got revoked or permission changed, mark failed
  ...
  const ops: AnyBulkWriteOperation<any>[] = pulled.rows.map((r) => ({
    updateOne: {
  ...
  // Bulk write (unordered for speed)
  if (ops.length > 0) {
    await costCol.bulkWrite(ops, { ordered: false });
  }

  // Mark sync success on connection doc
  await connCol.updateOne(
    { userId },
    {
      $set: {
